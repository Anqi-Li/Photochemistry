...
def set_Js(t_s, js_ds):
    ...
    # plt.figure()
    # reaction_lst[3].rate_constant.value.plot(y='z')
    # plt.show()
...
# with xr.open_dataset('./ode_result_{}_{}_{}.nc'.format(55, 127, 50)) as ds:
#     t_s = ds.t.sel(t=lst, method='nearest').values * h2s
#     [s.set_density(ds[s.name].sel(t=lst, method='nearest').interp(z=z)).set_unit('cm-3')
#      for s in (o3, o, h, h2o2, oh,  ho2)]
# #setphotolysis rates
# set_Js(t_s, js_ds)

# # set_time_dependent_variables(t_s=t_s)
# # Js = check_js(t_s=t_s, ds=ds)
# # fig, ax = plt.subplots(2,2)
# # Js.J1.plot(y='z', ax=ax.ravel()[0])
# # Js.J2.plot(y='z', ax=ax.ravel()[2])
# # Js.J3.plot(y='z', ax=ax.ravel()[1])
# # Js.J4.plot(y='z', ax=ax.ravel()[3])
# # plt.suptitle('sza = {}'.format(func_sza(t_s)))
...
# with xr.open_dataset('./ode_result_{}_{}_{}.nc'.format(55, 127, 50)) as ds:
#     ds.OH.sel(t=t_s*s2h).plot(y='z', xscale='log', marker='*', label='model')
# plt.legend()
...
# with xr.open_dataset('./ode_result_{}_{}_{}.nc'.format(55, 127, 50)) as ds:
#     ds.HO2.sel(t=t_s*s2h).plot(y='z', xscale='log', marker='*', label='model')
# plt.legend()
...
def balance(species_query_densities, t_s, 
    ...
    for i,s in enumerate(species_query):
        s.set_density(xr.DataArray(species_query_density_lst[i], dims=('z',), coords=(z,)))
...
species_query = [o3, o, h, h2o2, ho2, oh]
...
# with xr.open_dataset('./ode_result_{}_{}_{}_path_modified_no_switch_interp.nc'.format(198, 223, 50)) as ds:
...
# ode_result = xr.Dataset({s.name: data for s, data in zip(species_query, data_lst) })
# ode_result = ode_result.assign(t=ode_result.t*s2h)
# ode_result.to_netcdf('./ode_result_{}_{}_{}_pre_Js.nc'.format(int(t_start_new*s2h), int(t_end_new*s2h), dt))
